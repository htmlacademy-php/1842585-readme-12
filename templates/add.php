 ...
 * @var $part_template string - шаблон для добавления поста
    ...
    <div class="adding-post container">
        <div class="adding-post__tabs-wrapper tabs">
                ...
                <ul class="adding-post__tabs-list filters__list tabs__list">
                    <?php foreach ($post_types as $post_type) : ?>
                        <li class="adding-post__tabs-item filters__item">
                            <a
                                href="add.php?type_id=<?= htmlspecialchars($post_type['id']) ?>"
                                class="adding-post__tabs-link filters__button
                                        filters__button--<?= htmlspecialchars($post_type['icon_class']) ?>
                                        tabs__item button <?= htmlspecialchars($post_type['id']) === $type_id
                                        ? "filters__button--active tabs__item--active" : "" ?>">
                                <svg class="filters__icon" width="22" height="18">
                                    <use xlink:href="#icon-filter-<?= htmlspecialchars($post_type['icon_class']) ?>">
                                    </use>
                                </svg>
            ...
            <div class="adding-post__tab-content">
                <section class="adding-post__<?= htmlspecialchars($current_type["icon_class"]) ?>
                                tabs__content tabs__content--active">
                    <h2 class="visually-hidden">Форма добавления ссылки</h2>
                    <form
                        class="adding-post__form form"
                        action="add.php?type_id=<?= htmlspecialchars($current_type['id']) ?>"
                        method="post"
